 ...
 * @version 1.0
 */
public class Gui_AdminLog extends JFrame implements ActionListener {
	 ...
	 */
	public Gui_AdminLog(Object pro) {
		this.pro=(ProvSy) pro;
		//Gui_adminLog frame = new Gui_adminLog();
		...
		Label1 = new JLabel("As an administrator, please input your id and password:");
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(event.getSource() == check){
			...
			login=Meter.read("resource\\AdministratorInfo.txt");
			...
			for(int i=0; i<login.size();i++){
				...
				if(dataFile[0].equals(idText)==true){
					sequence = i;// dataFile[sequence] stored i		
					if(dataFile[1].equals(passText)==true)
						flagPass=1;
				}
			}
					
			//if((idText.equals(id))==false||(passText.equals(password))==false){
			if(flagPass !=1){
				...
				new Gui_AdminFunction(this.pro,sequence);
				dispose();
			}
		}
